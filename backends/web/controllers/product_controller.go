...
type ProductController struct {
	...
	ProductService services.IProductService // Service逻辑处理
}

func NewProductController() *ProductController {
	db, _ := common.NewMysqlConn()
	repositoryProduct := repository.NewProductManager("product", db)
	return &ProductController{ProductService: services.NewIPoductSeviceManager(repositoryProduct)}
}
...
// 显示所有商品 Get /product/all 显示商品详情页面：获取所有商品
...
// 添加商品页面 GET /product/add ：add.html
...
func (p *ProductController) PostAdd() {
	...
	p.Ctx.Application().Logger().Info("成功添加商品, ID为", id)
	p.Ctx.Redirect("/product/all")
}

// 修改商品页面 GET /product/manager：展示多条商品，页面有修改and删除操作按钮
...
// 修改商品表单提交 POST /product/update ： 提交修改商品表单
